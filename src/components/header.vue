<template>
  ...
  <div></div>
  <div class="flex space-x-3 items-center justify-end px-3 pr-5">
    <div></div> <!-- Add an empty div for spacing -->
    
    <div class="text-md">{{ username }}</div>
    <Avatar icon="" class="mr-2" style="background-color:#16cbe7; color: #ffffff" shape="circle"
...
export default {
  ...
  methods: {
    ...
    async getUsernameFromId() {
      ...
      try {
        const response = await axios.get(`https://backend-blue-nine-65.vercel.app/user/${id}`, config);
    ...
    showProfileDialog() {
      ...
      axios
        .get(`https://backend-blue-nine-65.vercel.app/user/${id}`, config)
        .then((response) => {
          const data = response.data;
    ...
    async saveProfile() {
      try {
        ...
        const response = await axios.put(`https://backend-blue-nine-65.vercel.app/user/${id}`, updatedProfile, config);
